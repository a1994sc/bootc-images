...
func (v *Volume) AddDirectory(ctx context.Context, folder, ref string) error {
	...
	go func() {
		...
		for {
			select {
			case <-ticker.C:
				fmt.Println("adding image volume layers", "count", added.Load(), "path", folder)
			case <-stopTicker:
				return
			}
		}
	...
	fmt.Println("built image volume", "ref", ref, "digest", manifestDesc.Digest, "layers", len(v.layers), "duration", time.Since(start).Round(time.Millisecond))
	return nil
}
